...
export default function DiwaliFlowerpot({
  ...
  useEffect(() => {
    ...
    class Spark {
      constructor() {
        this.x = w / 2 + (Math.random() - 0.5) * 18;
        this.y = h * 0.52;
        const angle = -Math.PI / 2 + (Math.random() - 0.5) * 0.8;
        const speed = (7 + Math.random() * 10) * scale;
    ...
    const loop = () => {
      ...
      if (active) {
        for (let i = 0; i < 10; i++) particles.push(new Spark());
      }
  ...
  }, [active, scale, primaryColor, sparkColor]);
  ...
  return (
    <div
      ...
      style={{
        width: `${460 * scale}px`,
        height: `${540 * scale}px`,
        animation: active ? "diwaliFloat 3.5s ease-in-out infinite" : "none",
      }}
    >
      <canvas ref={canvasRef} className="pointer-events-none absolute inset-0 z-20" />

      <div className="relative z-10 mt-auto mb-8 w-full">
        {active && (
          <div className="pointer-events-none absolute -top-16 left-1/2 w-[70%] -translate-x-1/2 animate-pulse">
            <svg viewBox="0 0 200 200" className="h-full w-full fill-amber-400 opacity-90">
