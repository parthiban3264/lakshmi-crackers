...
export default function DiwaliFlowerpot({
  ...
  // Generate unique IDs so mobile WebKit never breaks gradient links
  const uniqueId = useId().replace(/:/g, "");
  const bodyGradId = `bodyGrad_${uniqueId}`;
  const starGradId = `starGrad_${uniqueId}`;
      ...
      >
        ...
        <div className="relative z-10 w-[260px] pb-4">
          ...
          <svg viewBox="0 0 260 280" className="h-full w-full drop-shadow-2xl">
            ...
            {/* Fallback solid fill color added (#f59e0b) alongside dynamic url */}
            <path
              d="M98 30 L162 30 L220 230 C220 250 40 250 40 230 Z"
              fill={`url(#${bodyGradId})`}
            ...
            <polygon points="130,118 134,130 147,130 137,138 141,150 130,142 119,150 123,138 113,130 126,130" fill={`url(#${starGradId})`} className="fill-red-500" />
            <polygon points="98,142 101,150 110,150 103,156 106,165 98,159 90,165 93,156 86,150 95,150" fill={`url(#${starGradId})`} className="fill-red-500" />
            <polygon points="162,142 165,150 174,150 167,156 170,165 162,159 154,165 157,156 150,150 159,150" fill={`url(#${starGradId})`} className="fill-red-500" />
          </svg>
        </div>
      </div>
    </div>
